...
import { deleteRenderTask } from "@/lib/delete-render-task";
...
function resolveHistoryImage(task) {
  ...
  if (resultImageUrl.includes("mock-render.local")) {
  ...
  const historyCards = useMemo(
    () =>
      tasks.map((task) => {
        ...
        const rawPrompt = normalizePrompt(task.prompt || task.renderPrompt);
